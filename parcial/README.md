# Retail-E-Commerce — Parcial

Copia del código del proyecto en su estado actual: un login de tienda
virtual (Flask + MySQL) que implementa los 5 patrones creacionales —
...
> (en la raíz del repo). Esta guía es solo para dejar el proyecto corriendo
> **desde esta carpeta `parcial/`**.

## 📂 Estás en la carpeta correcta

A diferencia de `tienda_virtual/tienda_virtual/` (que tiene una carpeta
contenedora repetida), aquí todo está plano — `app.py` está justo en esta
misma carpeta:
...
1. **Clonar el repo y entrar a esta carpeta:**
   ...
   cd Sector-Retail-E-Commerce/parcial
   ```
